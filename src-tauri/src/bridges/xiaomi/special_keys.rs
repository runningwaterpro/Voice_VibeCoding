    ...
    unsafe extern "system" fn proc(code: i32, wparam: WPARAM, lparam: LPARAM) -> LRESULT {
        ...
        if code >= 0 {
            ...
            let suppress = match vk {
                ...
                0xA6 if direct_signal_recent("back", Duration::from_millis(250))
                    || direct_signal_recent("0x00F1", Duration::from_millis(250)) => Some("back"),
                0x24 | 0xAC
                    if should_suppress_native_menu_home(
                        vk as u16,
                        tap_ready,
                        direct_signal_recent("home", Duration::from_millis(250))
                            || direct_signal_recent("0x004A", Duration::from_millis(250)),
                    ) =>
                {
                    Some("home")
                }
                ...
                0x5F | 0xFF if direct_signal_recent("power", Duration::from_millis(250))
                    || direct_signal_recent("0x0066", Duration::from_millis(250)) =>
                {
                    Some("power")
                }
                _ if scan == 0x5E && direct_signal_recent("power", Duration::from_millis(250))
                    || scan == 0x5E && direct_signal_recent("0x0066", Duration::from_millis(250)) =>
                {
                    Some("power")
                }
